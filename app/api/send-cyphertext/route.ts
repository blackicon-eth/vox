...
import { JsonRpcProvider, Wallet, Contract, ethers, hexlify} from "ethers";
import {contractAddress, allocatorAddress} from '../../../lib/constants'
import {AttestationABI} from '../../../lib/abi/AttestationABI'
import {CoreIdABI, CoreIdAddress} from '../../../lib/abi/CoreIdABI'
import { FhenixClient, SupportedProvider, EncryptionTypes, EncryptedUint256 } from 'fhenixjs';


export const POST = async (req: NextRequest) => {
  const body = await req.json();
  const { taskId, schemaId, uHash, recipient, publicFieldsHash, validator, allocatorSignature, validatorSignature, publicFields, embeddedAddress} = body.data;

  if (!taskId || !schemaId || !uHash || !recipient || !publicFieldsHash || !validator || !allocatorSignature || !validatorSignature || !publicFields) {
    ...
    try {
      ...
      const args = {
        ...
        publicFieldsHash,
        validator,
        allocatorSignature, // Convert to bytes
        validatorSignature // Convert to bytes
    };
  ...
  try {
    ...
    const client = new FhenixClient({ provider: fhenixProvider as SupportedProvider });
    ...
    let castedName: EncryptedUint256 = await client.encrypt(1, EncryptionTypes.uint256);
    let castedSurname: EncryptedUint256 = await client.encrypt(1, EncryptionTypes.uint256);
    let castedUsername: EncryptedUint256 = await client.encrypt(1, EncryptionTypes.uint256);
    let castedDataNascita: EncryptedUint256 = await client.encrypt(1, EncryptionTypes.uint256);
